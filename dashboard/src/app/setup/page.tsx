...
import { Step1Business } from '@/components/setup/Step1Business'
...
import { API_URL, setTenantId } from '@/lib/api'
...
const STEPS = ['Thông tin doanh nghiệp', 'Kết nối OpenClaw', 'Thông báo', 'Hoàn tất']
    ...
    return (
      <div className="min-h-screen bg-[#f2f2f7] dark:bg-zinc-950 flex items-center justify-center p-4" >
        <div className="w-full max-w-md">
          ...
          <button
            onClick={() => setMode('new')}
            className="w-full bg-white dark:bg-zinc-900 dark:ring-1 dark:ring-white/5 rounded-2xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] py-3.5 text-sm font-medium text-blue-500 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors"
          >
    ...
    <div className="min-h-screen bg-[#f2f2f7] dark:bg-zinc-950 flex items-center justify-center p-4" >
      <div className="w-full max-w-xl">
        ...
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-zinc-100">Zalo Monitor</h1>
          <p className="text-gray-500 dark:text-zinc-400 mt-1 text-sm">Thiết lập lần đầu</p>
        </div>
        ...
        <div className="bg-white dark:bg-zinc-900 dark:ring-1 dark:ring-white/5 rounded-3xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] p-8">
          {step === 1 && <Step1Business onDone={(s) => { setSetup(s); setTenantId(s.tenantId); setStep(2) }} />}
          {step === 2 && <Step2Connect setup={setup} onDone={() => setStep(3)} />}
          {step === 3 && <Step3Notify setup={setup} onDone={() => setStep(4)} />}
          {step === 4 && <Step4Done setup={setup} />}
        </div>
