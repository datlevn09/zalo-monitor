  ...
  return (
    <div className="p-4 md:p-8 max-w-3xl mx-auto">
      ...
      <div className="bg-white dark:bg-zinc-900 dark:ring-1 dark:ring-white/5 rounded-2xl shadow-[0_1px_2px_rgba(0,0,0,0.04)] overflow-hidden divide-y divide-gray-100 dark:divide-white/5">
        {users.map(u => (
          <div key={u.id} className="px-4 py-3.5 flex items-center gap-3">
            ...
            {/* Assign groups button (STAFF only) */}
            {u.role === 'STAFF' && (
              <button
            ...
            {u.role !== 'OWNER' && (
              <button onClick={() => remove(u.id)} className="text-gray-400 dark:text-zinc-500 hover:text-red-500 transition-colors text-sm">🗑</button>
            )}
          </div>
        ))}
        {users.length === 0 && (
          <div className="px-4 py-8 text-center text-sm text-gray-400 dark:text-zinc-500">Chưa có thành viên</div>
        )}
      </div>
      ...
      <div className="mt-4 bg-blue-50 dark:bg-blue-500/10 rounded-xl px-4 py-3 text-xs text-blue-700 dark:text-blue-300 space-y-1">
        ...
        <p>👑 <b>Owner</b> — toàn quyền, xem tất cả nhóm</p>
        <p>👔 <b>Manager</b> — xem tất cả nhóm, quản lý staff</p>
        <p>👤 <b>Staff</b> — chỉ thấy nhóm được phân công + nhóm chưa phân công</p>
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={() => setShowInvite(false)}>
          <div className="bg-white dark:bg-zinc-900 dark:ring-1 dark:ring-white/5 rounded-3xl p-6 w-full max-w-md" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-xs text-gray-500 dark:text-zinc-400 mb-4">
              Hệ thống gửi email đặt mật khẩu cho người được mời (link hạn 7 ngày).
            </p>
            <div className="flex gap-2">
              <button onClick={() => setShowInvite(false)} className="flex-1 py-2.5 bg-gray-100 dark:bg-white/10 rounded-xl text-sm font-medium text-gray-700 dark:text-zinc-300">Huỷ</button>
              <button onClick={invite} className="flex-1 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-xl text-sm font-medium transition-colors">Gửi lời mời</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[80vh]">
            <div className="px-5 pt-5 pb-3 border-b border-gray-100 dark:border-white/10">
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setAssignUser(null)} className="text-gray-400 hover:text-gray-600 text-xl leading-none">×</button>
              </div>
            </div>

            <div className="flex-1 overflow-y-auto px-5 py-3 space-y-1.5">
              {allGroups.length === 0 && (
                <p className="text-center text-sm text-gray-400 py-8">Chưa có nhóm nào</p>
              )}
              {allGroups.map(g => {
                const checked = selected.has(g.id)
                return (
                  <label key={g.id} className={`flex items-center gap-3 px-3 py-2.5 rounded-xl cursor-pointer transition-colors ${
